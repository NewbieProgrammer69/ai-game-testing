"""Compare PPO and BC evaluation results side by side and save a summary CSV."""
...
def main():
    # Load per-episode evaluation logs written by evaluate_ppo.py and evaluate_bc.py.
    ppo_df = pd.read_csv(os.path.join(config.LOG_DIR, "ppo_eval_results.csv"))
    bc_df = pd.read_csv(os.path.join(config.LOG_DIR, "bc_eval_results.csv"))
    ...
    out_path = os.path.join(config.LOG_DIR, "comparison.csv")
    ...
    if ppo["mean_reward"] > bc["mean_reward"]:
        ...
        print("This suggests reinforcement learning produced a more robust policy than supervised imitation on this task.")
        ...
        print("Behavior Cloning successfully imitated the expert and matched (or beat) the source policy.")
    else:
        ...
        print("BC fully replicated the expert PPO policy's behavior on this task.")
