"""Collect expert demonstrations from the trained PPO agent for Behavior Cloning.

Only successful episodes (reward >= SUCCESS_THRESHOLD) are kept, so the BC
...
def main():
    # Load the trained PPO agent to act as the "expert".
    ...
    for ep in range(1, config.N_DEMO_EPISODES + 1):
        obs, _ = env.reset()
        done = False
        total_reward = 0.0
        ...
        # Only keep the episode if it met the success threshold.
    ...
    print(f"Episodes run       : {config.N_DEMO_EPISODES}")
