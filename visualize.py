"""Generate comparison plots for PPO vs BC evaluation and BC training curves."""
...
def load_data():
    ...
    bc_log = pd.read_csv(os.path.join(config.LOG_DIR, "bc_training_log.csv"))
...
def plot_reward_comparison(ppo_df, bc_df):
    ...
    ax.set_title("Mean Reward Comparison: PPO vs BC")
...
def plot_episode_length_comparison(ppo_df, bc_df):
    ...
    fig, ax = plt.subplots()
    ax.bar(labels, means, yerr=stds, capsize=8, color=[PPO_COLOR, BC_COLOR])
    ax.set_title("Mean Episode Length: PPO vs BC")
    ax.set_ylabel("Steps")
...
def plot_reward_distribution(ppo_df, bc_df):
    ...
    fig, ax = plt.subplots()
    bp = ax.boxplot(data, labels=["PPO", "BC"], patch_artist=True)
    for patch, color in zip(bp["boxes"], [PPO_COLOR, BC_COLOR]):
        patch.set_facecolor(color)
    ax.axhline(195, linestyle="--", color="gray", label="Success Threshold")
    ax.set_title("Reward Distribution: PPO vs BC")
...
def plot_bc_training_loss(bc_log):
    ...
    ax.set_title("Behavior Cloning Training and Validation Loss")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
    ax.legend()
    save_fig("bc_training_loss.png")
...
def plot_success_rate_comparison(ppo_df, bc_df):
    ...
    ax.set_title("Success Rate Comparison: PPO vs BC")
